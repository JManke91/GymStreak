...
//
//  IDENTICAL COPY of `GymStreak/Domain/Models/WeightUnit.swift` — the watch may
//  not import iOS `Domain/` (SwiftData), so this follows the repository's
//  per-target file-copy pattern. Keep the two byte-identical below the header:
//  a divergence here means the same stored kilograms read as different numbers
//  on the two devices. `GymStreakWatchTests/WeightUnitTests.swift` is the twin
//  of the iOS suite and exists to catch exactly that.
//
