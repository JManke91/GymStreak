...
// MARK: - Optimistic template fold

/// Folds a pending template transaction's set-only intent over a routine —
/// the watch's optimistic overlay. Mirrors the iOS ingestion rules: only sets
/// whose actual values differ from planned are applied; a swapped exercise's
/// values belong to the matching alternative's scheme, never the primary's.
enum WatchRoutineTemplateFold {
    static func apply(_ workout: CompletedWatchWorkout, to routine: WatchRoutine) -> WatchRoutine {
        ...
        let updatedExercises = routine.exercises.map { exercise -> WatchExercise in
        ...
        return WatchRoutine(id: routine.id, name: routine.name, exercises: updatedExercises)
    }
