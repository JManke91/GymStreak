...
struct WorkoutExerciseCatalogView: View {
    ...
    private var filteredItems: [WatchExerciseCatalogItem] {
        ...
        return catalogStore.items.filter {
            $0.name.localizedStandardContains(searchText)
                || $0.muscleGroups.contains { localizedWatchMuscleGroup($0).localizedStandardContains(searchText) }
                || localizedWatchEquipment($0.equipmentTypeRaw).localizedStandardContains(searchText)
        }
    }
    ...
    private func metadata(for item: WatchExerciseCatalogItem) -> String {
        let muscle = item.muscleGroups.first.map(localizedWatchMuscleGroup)
            ?? String(localized: "General")
        let equipment = localizedWatchEquipment(item.equipmentTypeRaw)
