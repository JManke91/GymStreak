...
- `PersonalRecordService` — Walks the session history chronologically and records a PR whenever a new per-exercise estimated-1RM maximum is reached. Returns `session.id → prCount` plus `session.id → {stableKey → PRDetail}`, where `PRDetail` carries the record itself: the achieving set's `setId`, weight, reps, the new estimated 1RM, and the previous best (nil on first-ever performance — a first performance counts as a PR). Consumers: `HistoryView` (counts for the list badges) and `WorkoutDetailView` (details).
...
- `WorkoutDetailView` — System Back button with native leading-edge swipe-back, type chip + date, routine name title, 4-metric stat grid, optional Apple Health banner (reads kcal async from HealthKit via `HKMetadataKeyExternalUUID`), notes section, per-exercise blocks via `WorkoutDetailExerciseBlock`. The transparent navigation bar keeps the editorial canvas while preserving the native navigation interaction; Edit is a trailing toolbar item. On appear, loads PR details (`[stableKey: PRDetail]` for this session) + HealthKit kcal + a `[UUID: ExerciseComparisonResult]` dictionary via `ExerciseProgressService.compareWithPrevious(workout:)`, keyed by `WorkoutExercise.id`. PR lookup is by `exercise.stableKey` — an earlier version matched by `exerciseName.lowercased()` while the service keyed by `stableKey` (which prefers `exerciseId.uuidString`), so the per-exercise PR badge silently never appeared for exercises linked to the library; fixed 2026-07-08.
