...
GymStreak/Presentation/Views/History/Components/WorkoutSessionHeaderView.swift  — extracted 2026-09-04
GymStreak/Presentation/Views/History/Components/WorkoutStatGrid.swift           — extracted 2026-09-04
GymStreak/Presentation/Views/History/WorkoutDetailExerciseDisplay.swift                 — added 2026-09-04
...
- `WorkoutSessionHeaderView` — Editorial session header: type chip + weekday/date + routine title. Takes three values; the screen formats the date off its hoisted `static let`.
- `WorkoutStatGrid` / `WorkoutStatTile` — The 4-metric row (Dauer / Sätze / Volumen / Intensität) and one tile of it. Values arrive as pre-formatted strings; each metric's icon and colour are fixed inside the component. Carries no outer margin — `WorkoutDetailView` supplies its 16 pt.
...
- `WorkoutCardView` — Date block + routine name + type chip + optional PR badge + metrics row + intensity ring. Tapping pushes `WorkoutDetailView`. Takes `card: WorkoutCardModel` **and `weightUnit: WeightUnit`** — the unit is passed rather than read from the environment because the view is `Equatable` for row skipping, and an `@Environment` property would break the synthesized `==` and leave it blind to a unit change (`docs/weight-unit-preference.md` §8).
...
`WorkoutCardView` and `HistoryCalendarView` render values and derive nothing — the one thing
they format is a weight, through the `WeightFormatting` seam, which converts canonical
kilograms into the user's unit off hoisted `static let` styles. The binding rules,
...
- `WorkoutDetailView` — System Back button with native leading-edge swipe-back, type chip + date, routine name title, the muscle map card (see [Muscle Map](./muscle-map.md)), 4-metric stat grid, optional Apple Health banner (reads kcal async from HealthKit via `HKMetadataKeyExternalUUID`), notes section, per-exercise blocks via `WorkoutDetailExerciseBlock`. The header and the stat grid are the extracted `WorkoutSessionHeaderView` and `WorkoutStatGrid` components (2026-09-04) — they were private helpers of this view until the onboarding history slide needed the shipped tiles rather than a redrawn copy. The transparent navigation bar keeps the editorial canvas while preserving the native navigation interaction; the trailing toolbar item is an ellipsis **Menu** holding Edit and a destructive Delete (see [Delete a Recorded Workout](./delete-workout.md)). On appear, loads PR details (`[WorkoutExercise.id: PRDetail]` for this session) + HealthKit kcal + a `[UUID: ExerciseComparisonResult]` dictionary via `await ExerciseProgressService.compareWithPrevious(workout:)`, both keyed by the concrete `WorkoutExercise.id` — the comparison dictionary now from each result's own `workoutExerciseId` rather than a positional `zip` (audit P1.6). This prevents one exercise-wide PR from rendering on every repeated occurrence.
- `MuscleMapCardView` (`Views/MuscleMap/` — it moved out of this folder when routine detail became its second caller) — "Trainierte Muskelgruppen": front and back schematic bodies with this workout's trained regions lit in the accent, plus a primary/secondary legend. Sits between the header and the stat grid. Takes a finished `MuscleMapCardModel` that `WorkoutDetailView` builds once in `loadMuscleMap()` (with `reading: .performed`, and passing the 16 pt `horizontalMargin` the card no longer owns itself); the aggregation walks `workoutExercises → sets` and must never run from `body`. The `ScrollView`'s `.defaultScrollAnchor(.top)` exists because of this card — see [Muscle Map](./muscle-map.md).
- `WorkoutDetailExerciseBlock` (`Views/History/Components/`) — Per-exercise card. Takes `display: WorkoutDetailExerciseDisplay` (`Views/History/WorkoutDetailExerciseDisplay.swift`) plus `prDetail: PersonalRecordService.PRDetail?` and `comparison: ExerciseComparisonResult?` — all three are value types, so no block body reads a `@Model`. The display resolves the planned-vs-actual choice (`progressiveOverloadApplied`) and the set ordering once, where `WorkoutDetailView` builds the rest of its display state; the block used to take the `WorkoutExercise` itself and walk `setsList` from a `body`-read property (rendering rule 4). Renders: title row (exercise name + optional PR/trophy badge + set count). When `prDetail` is present, a gold **PR record banner** (`PRRecordStrip`, own file in `Views/History/Components/`) follows: trophy + "New record: 87.5 kg × 8" / "Neuer Rekord: …" plus a secondary line "est. 1RM 111 kg · previous best 105 kg" (previous part omitted on first-ever performance). All three figures render in the user's unit; the PR set takes `WeightFormatting.label` and the two Epley estimates take `estimateLabel`, which rounds to whole display units. Then either a comparison strip (when previous session exists) or a "First session" badge. Then the sets grid: each set cell shows set number + weight (in the user's unit, or "BW"/"KG" for a bodyweight set) + reps, plus a `SetDeltaChip` below; the cell's VoiceOver value uses `WeightFormatting.spokenLabel`, so the screen reader says "kilograms"/"pounds" rather than "kay gee"; the **set that achieved the PR** gets a gold treatment (mini trophy next to the set label, gold-tinted background + border, VoiceOver appends "new personal record"). Note: an older tinted "best set" decoration was removed as cryptic (chat2 feedback) — the PR-set highlight is different and deliberate: it marks only genuine records and is explained by the adjacent record banner (user request 2026-07-08). PR gold is the shared `DesignSystem.Colors.pr` token (also used by `WorkoutCardView`).
- `ExerciseComparisonStrip` — Inline strip rendered between exercise title and sets grid. Shows `vs. <prev date>` + `Top` delta chip (top-weight delta vs `previousPerformance.bestSet.weight`, computed in kilograms and converted for display) + `Volume` delta chip (percentage delta vs `previousPerformance.totalVolume`). Its date formatter is a hoisted `@MainActor private static let`; it used to be allocated per strip per render. The previous performance comes from the same routine slot; legacy workouts use the conservative same-routine occurrence fallback above.
- `SetDeltaChip` — Capsule chip with SF Symbol arrow + value. Four states: `.gain(label, quantity)` (success green), `.loss(label, quantity)` (destructive red), `.neutral` (= symbol, no label), `.new` (sparkles + "New"/"Neu" for set positions with no previous counterpart). Weight delta wins over reps delta; reps delta only shown if weight matched exactly. Values use `.contentTransition(.numericText())` for animated number changes. Each set cell exposes VoiceOver via `accessibilityLabel` ("Set 2") + `accessibilityValue` ("85 kilograms, 8 reps, up 2.5 kilograms from last time") so the chip itself stays decorative. **`Delta.Quantity` (`.weight(spoken:)` / `.reps` / `.percentage`) is why the states carry two payloads:** the accessibility phrasing used to be chosen by substring-matching `"kg"` in the already-formatted label, which narrated a pound weight change as a rep change and a volume change as "up 0 reps" — see `docs/weight-unit-preference.md` §11.
