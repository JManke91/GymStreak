...
- **Dead code is not localized**: `InlineSetEditorView.swift`, `ValueStepperView.swift`, `WatchRoutinesViewModel.swift` (never instantiated), and the fully commented-out `SetNavigationBar.swift`. If any of these are revived, localize their strings then. (`ExerciseSetView.swift` and `RestTimerEditorSheet.swift` were on this list until they were deleted on 2026-07-25.) Same for `WatchWorkoutViewModel.errorMessage` — set in three places but never bound to any UI.
