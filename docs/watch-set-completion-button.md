...
0. **Top zone** (`WorkoutTopProgressView`, added 2026-07-12 from the Final design, handoff §4): the current exercise name (bold, `lineLimit(1)` + `minimumScaleFactor(0.85)`), the whole-workout completion percent (bright green, monospaced, `contentTransition(.numericText())`), and a per-exercise segment bar below — one segment per exercise in workout order, each a dark `segmentTrack` (#2A2A2C) capsule with a leading `accentGreen` fill sized to that exercise's completed/total sets (a fully-done exercise reads as a solid green pill). Fills its previously-empty space between the toolbar (clock/elapsed chip) and the stepper cluster.
...
- **`WorkoutTopProgressView.swift`**: top zone (exercise name + workout percent + per-exercise segment bar); pure display, reads only its init params
- **`FullScreenSetEditorView.swift`**: screen layout, shared steppers, done-flash state, rest/elapsed toolbar status; derives `exerciseProgress` (per-exercise completion fractions) from `viewModel.exercises` for the top zone
