...
`ProgressiveOverloadIncrement` lives alongside it and is the **weight-step scale**, so the surfaces cannot drift apart again (they did: the watch gained 0.5 first, on 2026-07-27). Since the weight-unit work (2026-08-27) it holds **one grid per display unit**, reached through `grid(for:)`:

| | `options` (presets) | `defaultOption` | `minimum` / `maximum` / `step` (free selection) |
| --- | --- | --- | --- |
| kg | 0.5, 1.25, 2.5, 5 | 2.5 | 0.25 / 50 / 0.25 |
| lb | 1.25, 2.5, 5, 10 | 5 | 0.25 / 100 / 0.25 |

Every value is in that unit's own **display space**. iOS `WeightIncreaseSheet` renders the current unit's presets as a radio list and converts the pick to kilograms exactly once, on apply; the watch offers them as quick-select chips and uses the free-selection bounds for its Digital Crown (2026-07-29). `normalized(_:in:)` clamps and snaps to that unit's grid.

**The pound grid is selected, not converted.** Converting the kilogram list gives 1.1 / 2.76 / 5.51 / 11.02 lb, which is nonsense on a plate rack — see `docs/weight-unit-preference.md` §9.

Three constraints worth keeping: the 0.25 stride exists in **both** units so every preset — 1.25 included — lands exactly on the grid and stays reachable/highlightable; the maximum is finite on purpose (Apple's unbounded `digitalCrownRotation(_:onChange:onIdle:)` overload carries **no stride and no haptic detents**, so it is the wrong tool for a stepped value, and the bounded/strided overload with a generous ceiling is the idiomatic choice); and a step is displayed with two fraction digits *whatever the unit*, via `WeightFormatting.incrementLabel`. `%.2g` (two *significant* digits) rounds 1.25 to a misleading "1.2" (shipped on both surfaces before 2026-07-28) — and so does the pound **weight** style, whose single decimal would turn a valid 1.25 lb step into "1.3".

**Watch drift, time-boxed.** The watch's copy of the file still carries the old kilogram-only flat API (`options`/`default`/`minimum`/`maximum`/`step`, `normalized(_:)`); ticket 04 of the weight-unit work brings it in line.
...
| `WeightIncreaseSheet` | Views/Components/WeightIncreaseSheet.swift | Bottom sheet for selecting weight increment. Reads `ProgressiveOverloadIncrement.grid(for:).options` for the user's unit (+0.5/+1.25/+2.5/+5 kg, or +1.25/+2.5/+5/+10 lb) and converts the pick to kilograms once, on apply — everything below it takes canonical kilograms. Value-based with two inits: `init(routineExercise:)` shows template values (routine editor), `init(workoutExercise:)` shows performed actual values (active workout + completion screen — correct for swapped exercises, whose weights never live on the primary template sets) |
