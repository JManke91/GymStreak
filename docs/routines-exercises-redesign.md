...
- **An exercise already in the routine can be added again (2026-08-24).** The "Bereits in Routine" section used to render its rows dimmed (`opacity 0.45`), checkmark instead of plus, and with no `NavigationLink` — the exercise was simply unaddable. That was wrong: **one exercise legitimately belongs in a routine twice**, e.g. a heavy 4–6 block and a light 8–12 block, which is exactly the case the chart's usage picker exists to separate (`docs/progress-charts.md`). Both sections now render the identical tappable row and lead to `ConfigureExerciseSetsView`; the section heading is what still tells the user the exercise is already in the routine, and the VoiceOver label/hint (`add_to_routine.already_added.a11y_label` / `.a11y_hint`, EN + DE) says it is already there and that adding it again creates a second entry with its own sets and rep goal. Nothing downstream needed changing — `RoutinesViewModel.addConfiguredExercise` appends a new `RoutineExercise` per call and never deduplicates by `Exercise`, and `PendingRoutineExercise` carries its own `UUID`, so two draft entries of one exercise stay distinct. `pickerRow(_:)` lost its `disabled` parameter with the last dimmed caller. Verified across the downstream consumers: everything that has to tell two entries apart keys on the **slot** (`RoutineExercise.id`) — `WorkoutExercise.routineExerciseId`, `WorkoutViewModel`'s origin lookup, `RoutineTemplateSyncService.matchingRoutineExercise`, the watch template transaction and `Routine.toWatchRoutine()`, and superset grouping — so duplicate slots run and sync correctly on both platforms.

**Known inconsistency, deliberately not changed:** the *mid-workout* add paths still refuse an exercise that is already in the workout — `AddExerciseToWorkoutView` on iOS and `WatchWorkoutStructuralReducer.isAlreadyActive` on the watch, both with the dimmed + checkmark treatment removed here. The same rationale would apply to them; it was simply out of the requested scope. Also worth knowing: two slots of one exercise in one routine collapse into a single AI-coach recap entry (those aggregators group by `WorkoutExercise.stableKey`, i.e. the exercise), and `ExercisesViewModel.usages(for:)` dedupes by routine, so `ExerciseDetailView`'s "Verwendet in" lists such a routine once. If the two slots are given the *same* rep goal, the chart's usage labels collide and fall back to `· #1` / `· #2` — unique but weakly informative.
...
### Derived data
- **Last trained per routine** (`RoutinesViewModel.lastPerformedByRoutine`): built from `workoutSessionRepository.lastCompletedStartDates(forRoutineIds:)` — one `LIMIT 1` query per routine, not a scan of the whole history (it previously folded `fetchCompleted()` in Swift; see `docs/history-performance.md` §1.2a for why N bounded queries and what was rejected). Drives the hero selection and card relative dates. Rebuilt on every `fetchRoutines()`.
