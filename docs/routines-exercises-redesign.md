...
Rewritten from a `List(.insetGrouped)` with a nav-bar Edit button into a `List(.plain)` on the dark canvas with a custom top bar (back / Bearbeiten / ellipsis menu). Every exercise is a self-contained rounded **card** instead of a grouped row.
...
- **Exercise card** (`normalExerciseCard`): `ExerciseHeaderView` (avatar, name, set summary, equipment tag, alternatives affordance, actions menu, superset badge) + always-visible info chips (Pause / Ziel / Alternativen via `MetaChipView`) + expandable body. Expansion shows either the normal set list (`RoutineSetRowView`), the set-reorder editor, or the alternatives editor — **all three legacy edit modes preserved**.
