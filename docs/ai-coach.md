...
- **Identity only.** `ExerciseProgressAggregator.buildProgress` additionally filters by `loadBehavior` and by the selected usage; the deep-dive does neither. A coach session count can therefore still exceed the chart's for reasons unrelated to the name — a uniquely-named exercise whose load behaviour changed after some workouts were logged, or a screen with a single usage selected. Not addressed here; the shared rule covers *which exercise a row belongs to*, not *which of its series a row joins*.
- **Both fetches run on the main actor.** `buildInput` is synchronous and nonisolated, and its only caller (`ExerciseDeepDiveViewModel.run`) is `@MainActor`, so under SE-0461 the whole aggregation — the unbounded `FetchDescriptor<WorkoutSession>` at the top, the per-row `workoutExercisesList`/`setsList` walks, and now the bounded library fetch — executes on main. This predates the identity fix and was not introduced by it; the added fetch is library-sized with no relationship traversal. It is user-tap-initiated behind the `.preparing` skeleton rather than `onAppear` work, which is why it has not surfaced as a hang. **The right fix is to move `buildInput` behind a model actor (or have the caller pass the already-fetched library), as `SwiftDataHistorySnapshotStore` does for the chart** — deliberately not done in this slice, which was a correctness fix.
