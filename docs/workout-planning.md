...
These plans are also what **Apple Calendar sync** publishes — an opt-in feature with its own doc, `docs/calendar-sync.md`. It is complete: the opt-in toggle, the app-owned calendar, **`.everyNDays` plans** as a rolling window of all-day events, **`.weekdays` plans** as one open-ended weekly `EKRecurrenceRule`, and refreshes on plan edits, completions and app activation. Both shapes read their dates from the helpers below — `upcomingCadenceDates` for the cadence window, `nextDue` for the series start — so the calendar always agrees with the planning sheet's preview and the routine cards. The calendar never consults the entitlement, so a lapsed subscriber's weekday series keeps being maintained, exactly as their weekly goal and day-strip markers are.
...
A signal that genuinely proves an import landed means observing `NSPersistentCloudKitContainer.eventChangedNotification` for an `.import` event with `endDate != nil && succeeded` *in this session*. **That signal now exists**: `CloudSyncStatus.hasCompletedImportThisSession`, added 2026-08-26 for `DefaultContentSeeder.recoverStrandedLibraryIfNeeded()`, which was gating on the weaker `lastSuccessfulSync` and got it wrong in both directions (see docs/starter-exercise-library.md → "Stranded-library recovery"). This pass was deliberately **not** switched over to it: the seeder's action is *conditioned* on an empty store, so an optimistic `.upToDate` makes it upload a duplicate catalog, whereas this repair *no-ops* on an empty store (`guard !schedules.isEmpty`) and simply retries next launch. The stronger gate is available here if this pass ever grows an action that fires on emptiness.
