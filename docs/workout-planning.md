...
- Architecture: a `Domain/Interfaces/WorkoutReminderScheduling` protocol + `Data/Notifications/` gateway wrapping `UNUserNotificationCenter`, wired in `AppDependencies` (mirrors the HealthKit/WatchSync gateway pattern).
