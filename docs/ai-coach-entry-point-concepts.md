...
- **Chat toolbar**: xmark dismiss (leading), gear → `AICoachSettingsView` pushed inside the chat's own NavigationStack (trailing). History header keeps its gear (settings only); its sparkle chat entry from the Phase 3 morning build was removed the same day.
