...
- **Compilation is a completion gate.** After any code change, build the actual `GymStreak` iOS scheme for an iOS destination and do not report the work complete until it returns `BUILD SUCCEEDED`. Building only the watch target, a partial target, or relying on an earlier build is not sufficient. If compilation cannot be run or fails, state that plainly and continue fixing it rather than calling the work complete.
...
**Run the `architecture-reviewer` agent** (`.Codex/agents/architecture-reviewer.md`, via the Agent tool on the current diff) when ANY of these apply:
...
1. Launch the **`ios-api-researcher`** agent (`.codex/agents/ios-api-researcher.toml`) via the Agent tool — do NOT answer from training data.
...
- **Guide auto-compaction, don't fight it.** Auto-compact preserves this AGENTS.md. Trust it for gradual context pressure; reserve an explicit clear/compact for clean task boundaries.
