...
final class ExerciseDeepDiveViewModel {
    ...
    func checkCache(exercise: Exercise, locale: Locale, modelContext: ModelContext) async {
        guard AICoachPreferences.shared.isExerciseDeepDiveEffectivelyEnabled,
        ...
        if let cached = AICoachCache.shared.loadExerciseDeepDive(key: key) {
    ...
    private func run(
        ...
        // 2. Preferences check
        guard AICoachPreferences.shared.isExerciseDeepDiveEffectivelyEnabled else {
            state = .unavailable
            return
        }
        ...
        if !bypassCache {
            if let key = cacheKey(exerciseId: exercise.id, modelContext: modelContext),
               let cached = AICoachCache.shared.loadExerciseDeepDive(key: key) {
                logger.debug("Cache hit for exercise deep-dive \(exercise.id, privacy: .private)")
                state = .success(text: cached.narrative, isCached: true)
                return
            }
    ...
    private func stream(
        ...
        do {
            guard let responseStream = try await AICoachService.shared.streamExerciseDeepDive(input: input) else {
                state = .unavailable
                return
            }
            ...
            if let key = cacheKey(exerciseId: exerciseId, modelContext: modelContext) {
                AICoachCache.shared.saveExerciseDeepDive(
                    key: key,
