...
//
//  This iOS copy additionally carries the Wire→Domain mapper at the bottom
//  (same split as `WatchModels.swift`): the watch has no Domain layer, so that
//  section must NOT be copied over.
//
...
// MARK: - Wire DTO to Domain ingestion input (iOS only)

// Keeps the Codable sync type out of `Domain/`, exactly as
// `CompletedWatchWorkout.toIncomingWatchWorkout()` does for the
// completed-workout kind. Called at the WatchConnectivity boundary before the
// payload reaches a Domain service.
//
// The mapping is deliberately TOTAL: an unsupported schema is carried as
// `isSchemaSupported == false` rather than mapped to nil, so the receiver can
// still identify the transaction and answer it with a versioned terminal
// rejection instead of leaving the watch's entry pending forever.

extension WatchProgressiveOverloadIntent {
    func toIncomingProgressiveOverload() -> IncomingProgressiveOverload {
        IncomingProgressiveOverload(
            isSchemaSupported: schemaVersion == Self.currentSchemaVersion,
            routineExerciseID: routineExerciseID,
            alternativeID: alternativeID,
            targetRepMin: targetRepMin,
            setChanges: setChanges.map {
                IncomingTemplateSetChange(
                    setID: $0.setID,
                    expectedReps: $0.expectedReps,
                    expectedWeight: $0.expectedWeight,
                    proposedReps: $0.proposedReps,
                    proposedWeight: $0.proposedWeight
                )
            },
            // Carried only when the sender supplied BOTH halves. A half-filled
            // correlation identifies nothing, and `isWellFormed` already
            // rejects it — mapping it to nil keeps the Domain input from
            // having to re-check the pairing.
            sourceWorkout: sourceWorkoutID.flatMap { workoutID in
                sourceRoutineExerciseID.map {
                    IncomingProgressiveOverload.SourceWorkout(
                        workoutID: workoutID, routineExerciseID: $0
                    )
                }
            }
        )
