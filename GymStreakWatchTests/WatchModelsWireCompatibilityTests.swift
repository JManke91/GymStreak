...
//  GymStreakWatchTests
//
//  Watch-side twin of `GymStreakTests/WatchModelsWireCompatibilityTests.swift`.
...
//  property's default value — an absent key throws `keyNotFound`. On the watch
//  that is not a per-field problem: `CompletedWatchWorkout` is persisted inside
//  `WatchSyncStateStore`'s App-Group state file, and one undecodable entry
//  quarantines the entire outgoing queue.
//
//  Assertions are kept aligned with the iOS twin so a re-drift fails on one side.
//
...
struct WatchModelsWireCompatibilityTests {
    ...
    /// is absent, which is what a payload encoded by an older build looks like.
    ...
    /// An absent `loadBehaviorRaw` resolves to resistance wherever the watch
    /// reads it back — the fold hands it to `WatchExercise`, which defaults on read.
    @Test
    func absentLoadBehaviorResolvesToResistanceOnTheWatch() throws {
        let workout = try JSONDecoder().decode(
            CompletedWatchWorkout.self,
            from: Data(Self.minimalPayload.utf8)
        )
        let completed = try #require(workout.exercises.first)

        let slot = WatchExercise(
            id: completed.id,
            name: completed.name,
        ...
        #expect(slot.toActiveWorkoutExercise().loadBehaviorRaw == "resistance")
    }

    /// Widening the field to Optional must not change what the watch actually
    /// sends: a non-nil value is still encoded, so iOS keeps receiving the key.
    @Test
    func senderStillEmitsLoadBehaviorOnTheWire() throws {
        let active = ActiveWorkoutExercise(
            id: UUID(),
        ...
        let encoded = try JSONEncoder().encode(active.toCompletedExercise())
        let object = try #require(
            JSONSerialization.jsonObject(with: encoded) as? [String: Any]
        )

        #expect(object["loadBehaviorRaw"] as? String == "counterweightAssistance")
    }
